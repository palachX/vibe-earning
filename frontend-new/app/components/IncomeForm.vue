<template>
  <form class="space-y-3" @submit.prevent="onSubmit">
    <div class="grid grid-cols-1 gap-3 sm:grid-cols-3">
      <div>
        ...
        <input
          ...
          type="date"
          required
          class="w-full rounded-md border border-slate-700 bg-slate-900 px-3 py-2 text-sm text-slate-100 outline-none focus:border-emerald-500"
        >
      </div>
      <div>
    ...
    <button
      type="submit"
      class="inline-flex items-center rounded-md bg-emerald-500 px-4 py-2 text-sm font-medium text-emerald-950 hover:bg-emerald-400 disabled:opacity-60"
      :disabled="submitting"
    >
